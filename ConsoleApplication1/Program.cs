...
namespace ConsoleApplication1
{
    class Program
    {
        ...
        Console.Title = "LeakedSource Lookup";
            ...
            var postData = $"&search={searchString}";postData += $"&searchType={searchType}";postData += $"&submit=Search";if (wildCard == "y") postData += $"&wildCard=true";
            var data = Encoding.ASCII.GetBytes(postData);request.Method = "POST";request.ContentType = "application/x-www-form-urlencoded";request.ContentLength = data.Length;
            ...
            if (responseString.Contains("hidden result"))
            {
                ...
                string RegMatch = "seconds.(.*?)function";
                if (rektHtml.Contains("The following results are provided free of charge!"))
                {
                    RegMatch = "seconds.(.*?)The following results are provided free of charge!";
                }
            ...
            Console.WriteLine($"It took us only {elapsedMs} ms to look this up!");
            goto doAgain;
        }
    }
}
